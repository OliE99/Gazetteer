    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          updateCountryCodeAndCountryName(lat, lng);
          

          return location;
        },
        () => {
          $("#selCountry").val().change();
          Toastify({
            text: "Could not get user location",
            duration: 3000,
            newWindow: true,
            close: true,
            gravity: "top",
            positionLeft: true,
            backgroundColor: "red",
          }).showToast("Error retrieving user country data", 4000, false);
        }
      );
    } else {
      $("#selCountry").val("GB").change();
    }
};
  ...
  states: [{
      ...
      onClick: function (btn, map) {
          ...
          $.ajax({
              ...
              dataType: 'JSON',
              data: {
                  country: $('#selCountry').val().replace("W. Sahara", "Morocco").replace("N. Cyprus", "Cyprus")
              },
              success: function (result) {
          ...
          $.ajax({
              ...
              dataType: 'JSON',
              data: {
                  country: $('#selCountry :selected').text().replaceAll(" ", "%20").replace("Dem.%20Rep.%20Korea", "North%20Korea").replace("Rep.", "Republic").replace("Lao%20PDR", "Laos").replace("Bosnia%20and%20Herz.", "Bosnia%20and%20Herzegovina").replace("Falkland%20Is.", "Falkland%20Islands").replace("Côte%20d'Ivoire", "Ivory%20Coast")
              },
              success: function (result) {
                  ...
                  if (result.status.name == "ok") {
                      ...
                      $("#countrytimezone").html(filterData[0]['annotations']['timezone']['offset_string']);
